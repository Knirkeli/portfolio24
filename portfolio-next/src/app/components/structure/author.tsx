...
interface Author {
  ...
  coverPhoto: string; // Replace 'string' with the correct type if known
  profilePhoto: string; // Replace 'string' with the correct type if known
  name: string;
  bio: {
    children: {
      text: string;
...
interface UseAuthorsReturn {
  authors: Author[];
  urlFor: (source: unknown) => { url: () => string }; // Adjust the type of 'source' if known
}

const AuthorComponent: React.FC = () => {
  const { authors, urlFor } = useAuthors() as UseAuthorsReturn;
